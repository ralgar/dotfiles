...
local function factory()
    ...
    -- Notify if cannot find the battery file
    if not fs.is_dir(pspath) then
        naughty.notify { text = "Battery Monitor: invalid power supply path", timeout = 0 }
        return
    end
    ...
    function bat.get_batteries()
        awful.spawn.with_line_callback("ls -1 " .. pspath, {
    ...
    bat_notification_low_preset = {
        ...
        fg           = beautiful.color.yellow,
        bg           = beautiful.bg_focusn,
        border_color = beautiful.color.yellow
    }
